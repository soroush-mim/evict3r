        ...
        for name_data, dataset in datasets_all.items():
            ...
            with accelerator.split_between_processes(list(range(len(dataset)))) as idxs:
                for data_idx in tqdm(idxs):
                    ...
                    if model_name == "stream3r" or "VGGT":
                        ...
                        with torch.cuda.amp.autocast(dtype=dtype):
                            with torch.no_grad():
                                ...
                                whole_run_peak_alloc = torch.cuda.max_memory_allocated()
                                whole_run_peak_res   = torch.cuda.max_memory_reserved()
                            preds, batch = results.ress, results.views
                            frame_num += len(batch)
                            run_time += (end - start)
                            run_peak_alloc = max(run_peak_alloc, whole_run_peak_alloc)
                            run_peak_res = max(run_peak_res, whole_run_peak_res)
                    ...
                    print(
                        f"frame num: {num_frames}",
                        file=open(log_file, "a"),
                    )
                    print(
                        f"whole_run_peak_alloc: {whole_run_peak_alloc/1024**3:.2f} GB",
                        file=open(log_file, "a"),
                    )
                    print(
                        f"whole_run_peak_res: {whole_run_peak_res/1024**3:.2f} GB",
                        file=open(log_file, "a"),
                    )
            ...
            # print(
            #     f"[MEM][rank={accelerator.process_index}] whole-run peak_alloc={whole_run_peak_alloc/1024**3:.2f} GB, "
            #     f"peak_reserved={whole_run_peak_res/1024**3:.2f} GB",
            #     file=open(log_file, "a")
            # )
            ...
            if accelerator.is_main_process:
                ...
                with open(osp.join(save_path, f"logs_all.txt"), "w") as f:
                    ...
                    f.write("\n")
                    # Report peak memory across all sequences (not averaged)
                    f.write(f"[MEM] whole-run peak allocated (max across sequences) = {run_peak_alloc/1024**3:.2f} GB\n")
                    f.write(f"[MEM] whole-run peak reserved (max across sequences) = {run_peak_res/1024**3:.2f} GB\n")
